...
## Файлы финальной интеграции

- `src/TwinTurbo/` — участник 1, совместимые импорты и команды без копирования классов.
- `src/TwinTurbo.ai/backtest.py` — участники 1/2, последовательный запуск подготовленных снимков.
- `src/TwinTurbo.ai/models/boosting.py` — участник 2, алгоритм перенесён интегратором из feat/forecast-models на единые признаки.
- `tests/test_integration_merge.py`, `docs/RELEASE_CHECK.md` — участник 1.
- Модельные инструкции и исторический fixture-отчёт из модельной ветки сохранены; актуальные команды обновлены при слиянии по поручению пользователя.

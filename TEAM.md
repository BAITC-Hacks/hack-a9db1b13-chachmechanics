...
Каркас предназначен для трёх участников. Все Python-файлы, конфигурационные заготовки, lock-файл и тесты пустые. Рабочего приложения пока нет.

Заполнены только README.md, TEAM.md, AGENTS.md, docs/CONTRACTS.md и .gitignore. Скрытые служебные файлы .gitkeep сохраняют пустые директории в Git.

## Участник 1 — данные, погода, время и интеграция

Ветка: `feat/data-replay`.
...
- pyproject.toml и requirements.lock пока пустые. Установка и команды приложения заработают только после реализации участником 1.
- demo/manifest.json пока пустой: его нельзя загружать как валидный JSON до заполнения. DEMO.md и DECISIONS.md также оставлены пустыми.
- docs/CONTRACTS.md не является реализованным API: сигнатуры ещё нужно закрепить в schemas.py.

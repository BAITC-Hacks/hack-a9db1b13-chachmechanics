    ...
    <main class="tt-main">
      ...
      <div class="tt-grid">
        <section class="tt-scene" aria-label="Интерактивная схема ветровой площадки"><canvas class="tt-canvas" aria-hidden="true"></canvas>
          <div class="tt-scene-top"><div class="tt-weather"><div class="tt-weather-top"><span class="tt-weather-temp">${num(weather?.temperature_c,0)}°</span><span class="tt-weather-icon">${icon('sun')}</span></div><div class="tt-weather-sub">${esc(weather?.condition || 'Нет погодных значений')}</div><div class="tt-weather-values"><span>${icon('wind')} ${num(weather?.wind_ms,1)} м/с</span><span>${esc(weather?.direction || '—')}</span></div><div class="tt-weather-sub">Прогноз на ${timeFmt(first.target_start)} UTC</div></div><div class="tt-scene-meta"><span>${seasonName[season]} · ${dateFmt(selection.origin_time,{year:'numeric'})}</span><small>${isDemo?'ДЕМОНСТРАЦИОННАЯ ПЛОЩАДКА':'УСЛОВНАЯ СХЕМА ПЛОЩАДКИ'}</small><small>2 ветротурбины</small></div></div>
